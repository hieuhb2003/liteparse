...
- **Layout & Formula Detection**:
  - Detect formulas with LaTeX recognition, tables, images, and more
  - PaddleX-based layout server (opt-in)
  - Formulas output as `<formula>LATEX</formula>`, tables wrapped in `<table>...</table>`
- **Flexible OCR System**:
...
Options:
  ...
  --layout-server-url <url> HTTP layout server URL (enables formula + table detection)
  --no-layout             Disable layout detection even if URL is set
...
LiteParse can detect structural elements (formulas, tables, images, titles) in PDF pages using a PaddleX-based layout server. When enabled, formulas are recognized and converted to LaTeX.

### Setup

Requires Python 3.12+, [uv](https://docs.astral.sh/uv/), and PaddlePaddle:

```bash
cd ocr/paddlex-layout
uv sync
uv run python server.py
# Server starts on http://localhost:8830
```
...
# Disable layout for a specific run
lit parse doc.pdf --layout-server-url http://localhost:8830/layout --no-layout
```
...
```typescript
const parser = new LiteParse({
  layoutServerUrl: 'http://localhost:8830/layout',
});
```

### Output

- Formulas are wrapped as `<formula>LATEX</formula>` with the recognized LaTeX string
- Tables are wrapped as `<table>...</table>` around their content

When layout is enabled, pages are processed **sequentially** (OCR → layout → next page) to avoid memory issues. Without layout, OCR runs in parallel as usual.

### API Specification

See [`LAYOUT_API_SPEC.md`](LAYOUT_API_SPEC.md) for the full layout server API specification. You can implement your own layout server in any language by following this spec.
...
- [PaddleX](https://github.com/PaddlePaddle/PaddleX) - Layout detection & formula recognition (optional)
