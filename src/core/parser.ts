...
export class LiteParse {
  ...
  constructor(userConfig: Partial<LiteParseConfig> = {}) {
    ...
    // Initialize layout engine (opt-in via layoutServerUrl)
  ...
  async parse(input: LiteParseInput, quiet = false): Promise<ParseResult> {
    ...
    try {
      ...
      if (this.layoutEngine && this.ocrEngine) {
        // Sequential per-page: OCR → layout → next page
        // Avoids OOM from parallel OCR when layout is also active
        log(`Running OCR + layout sequentially...`);
        ...
        try {
          ...
          for (const page of pages) {
            // OCR this page
            await this.processPageOcr(doc, page, log);

            // Layout detection for this page
            try {
              ...
              if (det.elements.length) {
                const formulas = det.elements.filter((e) => e.type === "formula");
                log(`  Layout page ${page.pageNum}: ${det.elements.length} elements (${formulas.length} formulas)`);
              }
        ...
        // Original flow: parallel OCR, then sequential layout
        if (this.ocrEngine) {
          await this.runOCR(doc, pages, log);
        }
        if (this.layoutEngine) {
          await this.runLayout(pages, pdfInput, log);
        }
      }
  ...
  /**
   * Run layout detection on all pages (after OCR, before grid projection).
   * Renders each page to an image via PdfiumRenderer, sends to layout server,
   * and mutates page.textItems with formula/table tags.
   */
  private async runLayout(
    ...
    // Sequential rendering: PdfiumRenderer uses shared WASM linear memory,
    // concurrent renderPageToBuffer calls would race on the shared HEAPU8 buffer.
    // Layout HTTP calls are the bottleneck anyway, not the rendering step.
    try {
      await renderer.init();
      await renderer.loadDocument(pdfInput, this.config.password);
