...
export interface LiteParseConfig {
  ...
  /**
   * URL of an HTTP layout detection server implementing the LiteParse Layout API.
   * When set (and `layoutEnabled` is not `false`), LiteParse renders each page
   * to an image and sends it to the server to detect formulas, tables, and other
   * structural elements. Detected formulas are replaced with LaTeX; tables are
   * wrapped in `<table>` tags.
   *
   * @see {@link https://github.com/run-llama/liteparse/blob/main/LAYOUT_API_SPEC.md | Layout API Specification}
   */
  layoutServerUrl?: string;

  /**
   * Whether layout detection is enabled. Only effective when `layoutServerUrl` is set.
   * Set to `false` to disable layout detection even when a server URL is configured.
   *
   * @defaultValue `true` when `layoutServerUrl` is provided
   */
...
export interface TextItem {
  ...
  confidence?: number;
  /** Whether this item was injected by layout detection as a formula replacement. */
  isFormula?: boolean;
  /** Whether this item is a `<table>` or `</table>` marker from layout detection. */
  isTableMarker?: boolean;
}
